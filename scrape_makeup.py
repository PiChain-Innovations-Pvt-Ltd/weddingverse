...
def generateUrl(csv):
    try:
        ...
        df = pd.read_csv(csv)
        venue_name = df['Artist Name'].tolist()
        venue_locations = df['Artist Location'].tolist()
        urls = []
        trimmed_loc = []
        ...
        for location, name in zip(trimmed_loc, venue_name):
            urls.append("https://www.weddingbazaar.com/bridal-makeup-artists/{}/{}".format(location.replace(' ', '-').lower(), name.replace(' ', '-').lower()))
        return urls
    except Exception as e:
...
async def scrape_data_to_csv(url):
    async with async_playwright() as p:
        try:
            ...
            artist_name = await page.locator('h3.mui-11hgfej').all_inner_texts()
            artist_location = await page.locator("p.mui-1vzi93v").all_inner_texts()
...
async def main(csv_file_loc="PLEASE PROVIDE CSV LOCATION"):
    ...
    with open(f"weddingbazaar/makeups/txt_loc_list/split_file_{sys.argv[1]}.txt") as file_in:
        urls = []
        for line in file_in:
            urls.append(line)
    try:
        for url in urls:
            ...
            csv_file = "weddingbazaar/makeups/csv_loc_data/{}.csv".format(url[30:-2])
            print(f"Writing makeup artist to CSV file: {csv_file}")
            await write_to_csv(data, csv_file)
            await csv_to_json(csv_file)

    # try:
    #     # URL of the page to scrape
    #     url = "https://www.weddingbazaar.com/bridal-makeup-artists/delhi/makeup-by-tanjam-chawla"
    #     final_data = json.dumps(await scrape_data(url), ensure_ascii=False)
    #     print(final_data)
