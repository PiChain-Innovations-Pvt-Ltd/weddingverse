...
def generateUrl(csv):
    try:
        ...
        df = pd.read_csv(csv)
        planner_name = df['Photographer Name'].tolist()
        planner_locations = df['Photographer Location'].tolist()
        urls = []
        trimmed_loc = []
        ...
        for location, name in zip(trimmed_loc, planner_name):
            urls.append("https://www.weddingbazaar.com/wedding-photographers/{}/{}".format(location.replace(' ', '-').lower(), name.replace(' ', '-').lower()))
        return urls
    except Exception as e:
...
async def scrape_data_to_csv(url):
    async with async_playwright() as p:
        try:
            ...
            photographer_name = await page.locator('h3.mui-11hgfej').all_inner_texts()
            photographer_location = await page.locator("p.mui-1vzi93v").all_inner_texts()
            ...
            return {
                ...
                "Photographer Location": photographer_location,
                "Price": price,
...
async def main(csv_file_loc="PLEASE PROVIDE CSV LOCATION"):
    ...
    with open(f"weddingbazaar/photographers/txt_loc_list/split_file_{sys.argv[1]}.txt") as file_in:
        urls = []
        for line in file_in:
            urls.append(line)
    try:
        for url in urls:
            ...
            csv_file = "weddingbazaar/photographers/csv_loc_data/{}.csv".format(url[30:-2])
            print(f"Writing photographer to CSV file: {csv_file}")
